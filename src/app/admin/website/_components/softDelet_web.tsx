...
const SoftDeleteWeb = ({ id }: { id: number }) => {
 ...
 mutate (`http://localhost:4000/website?q=`);
  ...
  const handleDelete = async () => {
    try {
      ...
      const response = await axios.delete(`http://localhost:4000/website/${id}`);
      ...
      if (error instanceof z.ZodError) {
        ...
        console.error("gagal hapus Website:", error);
      }
    }
  }
